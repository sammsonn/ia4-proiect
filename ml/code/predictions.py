...
if __name__ == '__main__':
    data = pd.read_csv('../csv/stockuri produse ia4.csv', low_memory=False)
    ...
    # point = len(unique_product_ids) // 20
    ...
    for product_id in product_ids:
        print(product_id, data[data['Product_ID'] == product_id]['Name'].iloc[0])
    ...
    product_details_df = product_details_df.head(40)
    print(product_details_df)

    # Try to import OpenAI's GPT-3 for generating recommendations
    import openai
    openai.api_key = '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'
